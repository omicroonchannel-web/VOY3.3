...
socket.on('update_dms', dms => {
    ...
    function getLocalPagesKey() {
        return me ? `voy_local_pages_${me.username}` : 'voy_local_pages_guest';
    }

    function loadLocalPages() {
        try {
            const raw = localStorage.getItem(getLocalPagesKey());
            const arr = raw ? JSON.parse(raw) : [];
            localPages = Array.isArray(arr) ? arr : [];
        } catch {
            localPages = [];
        }
    }

    function persistLocalPages() {
        localStorage.setItem(getLocalPagesKey(), JSON.stringify(localPages));
    }

    function compileTeaLocal(tea, title, domain) {
        const lines = String(tea || '').split(/?
/);
        const out = [];
        let curFont = '';
        for(const line of lines) {
            ...
            if(/^font:/i.test(t)) { curFont = t.slice(5).trim(); continue; }
            const open = curFont ? `<div style="font-family:${curFont.replace(/"/g,'')}">` : '';
            const close = curFont ? '</div>' : '';
            const esc = t.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
            if(/^h1:/i.test(t)) out.push(`${open}<h1>${esc.slice(3).trim()}</h1>${close}`);
            else if(/^h2:/i.test(t)) out.push(`${open}<h2>${esc.slice(3).trim()}</h2>${close}`);
            else if(/^h3:/i.test(t)) out.push(`${open}<h3>${esc.slice(3).trim()}</h3>${close}`);
            else if(/^link:/i.test(t)) {
                const raw = t.slice(5).trim();
                const parts = raw.split('|');
                const txt = (parts[0] || 'link').trim();
                const url = (parts[1] || parts[0] || '').trim();
                if(/^https?:\/\//i.test(url)) out.push(`${open}<p><a href="${url}" target="_blank" rel="noopener">${txt}</a></p>${close}`);
            } else if(/^img:/i.test(t)) {
                const u = t.slice(4).trim();
                if(/^https?:\/\//i.test(u)) out.push(`<img src="${u}" style="max-width:100%;border-radius:8px">`);
            } else {
                out.push(`${open}<p>${esc}</p>${close}`);
            }
        }
        return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title || domain || 'Local Page'}</title><style>body{margin:0;background:#111;color:#eee;font-family:ui-sans-serif,system-ui}main{max-width:920px;margin:0 auto;padding:24px}a{color:#80b8ff}</style></head><body><main><small>${domain || 'local'}</small>${out.join('\n')}</main></body></html>`;
    }

    function saveLocalPage() {
        const domain = (document.getElementById('pages-domain')?.value || '').trim().toLowerCase();
        const title = (document.getElementById('pages-title')?.value || '').trim();
        const tea = (document.getElementById('pages-tea')?.value || '').trim();
        if(!domain) return alert('Domain is required');
        if(!tea) return alert('TEA code is required');
        const renderedHtml = compileTeaLocal(tea, title, domain);
        const id = 'local_' + Date.now();
        const existingIdx = localPages.findIndex(x => x.domain === domain);
        const rec = { id, token: '', domain, title: title || domain, owner: me?.username || 'local', path: '', url: '', renderedHtml, updatedAt: new Date().toISOString(), local: true };
        if(existingIdx >= 0) localPages[existingIdx] = { ...localPages[existingIdx], ...rec };
        else localPages.unshift(rec);
        persistLocalPages();
        renderPages();
    }

    function deleteLocalPage(id, e) {
        if(e) e.stopPropagation();
        localPages = localPages.filter(x => x.id !== id);
        persistLocalPages();
        renderPages();
    }
    ...
    function renderPages() {
        ...
        const remote = (latestSites || []).map(x => ({ ...x, local: false }));
        const local = (localPages || []).map(x => ({ ...x, local: true }));
        const items = [...local, ...remote].filter(s => !q || (s.domain || '').toLowerCase().includes(q) || (s.title || '').toLowerCase().includes(q) || (s.owner || '').toLowerCase().includes(q));
        list.innerHTML = '';
        if(items.length === 0) {
        ...
        items.forEach(site => {
            ...
            const localBadge = site.local ? ' <span style="font-size:0.75em; opacity:0.7">[local]</span>' : '';
            sum.innerHTML = `<b>${site.domain}</b>${localBadge}<div style="font-size:0.8em; opacity:0.75">${site.title || ''} ? by ${site.owner || ''}</div>`;
            ...
            line.style = 'margin-top:6px; display:flex; gap:8px; align-items:center;';
            if(site.local) {
                const openBtn = document.createElement('button');
                openBtn.className = 'btn-small';
                openBtn.textContent = 'Open full page';
                openBtn.onclick = (e) => {
                    e.preventDefault();
                    const html = normalizeRenderedHtml(site.renderedHtml || '');
                ...
                delBtn.onclick = (e) => deleteLocalPage(site.id, e);
                line.appendChild(openBtn);
                line.appendChild(delBtn);
            } else {
                line.innerHTML = `<a href="/${site.path}" target="_blank" rel="noopener">Open full page</a>`;
            }
